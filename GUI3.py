...
class App(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        pages = (StartPage, CreateNewProject, LoadExistingProject,
                 LeafInterface)
        # Load all pages
        for F in pages:
...
class LoadExistingProject(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.option = tk.OptionMenu(self, self.variableProjectName,
    ...
    def refreshOptions(self, parent, controller):
        ...
        for choice in OPTIONS:
            self.option['menu'].add_command(
                label=choice,
                command=tk._setit(self.variableProjectName, choice))
...
class LeafInterface(tk.Frame):
    def __init__(self, parent, controller):
        ...
        labelProjectName = tk.Label(
            self,
            textvariable=self.variableProjectName,
            font=LARGE_FONT
        ).grid(row=0)

        labelCaterID = tk.Label(
            self,
            width=15,
            text="Caterpillar ID",
            anchor='w'
        ...
        entryLeafID = tk.Entry(
            self,
            width=50,
            textvariable=self.variableLeafID,
        ).grid(row=2, column=1)

        labelCaterInstar = tk.Label(
            self,
            width=15,
        ...
        entryCaterInstar = tk.Entry(
            self,
            width=50,
            textvariable=self.variableCaterInstar,
        ).grid(row=3, column=1)

        labelCaterAlive = tk.Label(
            self,
            width=15,
        ...
        image = image.resize((320, 256),
                             Image.ANTIALIAS)  ## The (x, y) is (width, height)
        ...
        self.labelImage3 = tk.Label(self, image=photo2, bg='gray')
    ...
    def getResponse(self, parent, controller):
        projectName = self.variableProjectName.get()[:-4]
        imageFileName = (projectName + "\\" + self.variableDate.get() + "." +
                         self.variableTime.get() + "CatID" +
                         self.variableCaterID.get() + "LeafID" +
                         self.variableLeafID.get() + ".png")
        self.image.save(imageFileName)

        imageOriginalFileName = (projectName + "\\" + self.variableDate.get() +
                                 "." + self.variableTime.get() + "CatID" +
                                 self.variableCaterID.get() + "LeafID" +
                                 self.variableLeafID.get() + "Original.png")
        self.imageOriginal.save(imageOriginalFileName)

        imageCroppedFileName = (projectName + "\\" + self.variableDate.get() +
                                "." + self.variableTime.get() + "CatID" +
                                self.variableCaterID.get() + "LeafID" +
                                self.variableLeafID.get() + "Cropped.png")
        ...
        imageSmall = self.image.resize(
            (320, 256), Image.ANTIALIAS)  ## The (250, 250) is (height, width)
        photo = ImageTk.PhotoImage(imageSmall)
        self.labelImage2.configure(image=photo)
        self.labelImage2.image = photo
